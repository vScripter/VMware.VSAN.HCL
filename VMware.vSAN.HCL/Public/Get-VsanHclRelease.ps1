...
.PARAMETER Raw
    Specify this switch parameter if you wish to return the raw content from the HCL DB .JSON file
.INPUTS
    System.String
.OUTPUTS
    System.Management.Automation.PSCustomObject
.EXAMPLE
    Get-VsanHclRelease -Verbose | Out-GridView
.EXAMPLE
    Get-VsanHclRelease -Verbose
.NOTES
...
#>
    ...
    PROCESS {
        ...
        if ($PSCmdlet.ParameterSetName.Contains('Raw')) {

            Write-Verbose -Message "[$($PSCmdlet.MyInvocation.MyCommand.Name)] Opening file { $FilePath }"
            try {

                Get-Content -Path $FilePath -Raw -ErrorAction 'Stop' | ConvertFrom-Json -ErrorAction 'Stop'
...
} # end function Get-VsanHclRelease